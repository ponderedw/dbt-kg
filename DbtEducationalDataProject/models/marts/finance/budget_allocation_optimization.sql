...
optimization_opportunities as (
    select
        ...
        case
            when budget_optimization_score >= 80 then round(allocated_budget * 1.1, 0)  -- 10% increase
            when budget_optimization_score >= 60 then allocated_budget  -- No change
            when budget_optimization_score >= 40 then round(allocated_budget * 0.95, 0)  -- 5% decrease
            else round(allocated_budget * 0.85, 0)  -- 15% decrease
...
portfolio_optimization as (
    select
        ...
        -- ROI projections
        case
            when budget_change_percentage > 0 then
                round(oo.graduates_per_100k_budget * 1.1, 2)  -- Projected 10% improvement
            when budget_change_percentage = 0 then
                oo.graduates_per_100k_budget
            else
                round(oo.graduates_per_100k_budget * 0.9, 2)  -- Potential 10% decline
        end as projected_graduate_productivity,
        
        case
            when budget_change_percentage > 0 then
                round(oo.revenue_to_budget_ratio * 1.05, 2)  -- Projected 5% improvement
            when budget_change_percentage = 0 then
                oo.revenue_to_budget_ratio
            else
                round(oo.revenue_to_budget_ratio * 1.02, 2)  -- Small improvement from efficiency

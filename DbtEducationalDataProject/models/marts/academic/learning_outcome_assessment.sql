...
course_outcome_effectiveness as (
    select
        clo.*,
        do.avg_mastery_rate as dept_avg_mastery_rate,
        do.avg_learning_growth as dept_avg_learning_growth,
        case
            when clo.pass_rate > do.avg_mastery_rate * 1.1 then 'Above Department Average'
            when clo.pass_rate < do.avg_mastery_rate * 0.9 then 'Below Department Average'
    ...
    left join departmental_outcomes do on clo.department_name = do.major
),
...
final_assessment as (
    select
        loa.*,
        do.major_avg_gpa,
        do.strong_learner_percentage as dept_strong_learner_rate,
        do.major_avg_difficulty as dept_avg_difficulty,
        case
    ...
    left join departmental_outcomes do on loa.major = do.major
)

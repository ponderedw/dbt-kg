...
with quarter_data as (
    select
        ...
        avg(e.grade_points) as avg_quarter_grade_points,
        avg(e.attendance_percentage) as avg_quarter_attendance,
    ...
    group by
        ...
        sem.start_date, sem.end_date, sem.quarter_duration_days, sem.quarter_status
),

trend_analysis as (
    select
        *,
        lag(total_enrollments) over (order by start_date) as prev_quarter_enrollments,
        lag(unique_students) over (order by start_date) as prev_quarter_students,
        lag(avg_quarter_grade_points) over (order by start_date) as prev_quarter_gpa,
        round(
...
seasonal_patterns as (
    select
        ...
        avg(avg_quarter_grade_points) as avg_gpa_by_season,
...
performance_categories as (
    select
        ...
        case
            when success_rate >= 85 then 'High Performing Quarter'
            when success_rate >= 70 then 'Good Performing Quarter'
            when success_rate >= 60 then 'Average Performing Quarter'
            else 'Low Performing Quarter'
        end as quarter_performance_category,
        case
    ...
    left join seasonal_patterns sp on ta.quarter_type = sp.quarter_type
)

...
student_collaboration_strength as (
    select
        ...
        min(student_a_gpa, student_b_gpa) as min_gpa,
        max(student_a_gpa, student_b_gpa) as max_gpa
...
faculty_collaboration_networks as (
    select
        ...
        f1.faculty_name as faculty_a_name,
        f2.faculty_name as faculty_b_name,
    ...
    group by 
        ...
        f1.department_name, f2.department_name, f1.department_id, f2.department_id
...
department_collaboration_metrics as (
    select
        department_name,
        count(distinct student_a) + count(distinct student_b) as unique_students_in_collaborations,
        avg(shared_courses) as avg_shared_courses_per_pair,
        count(case when connection_strength in ('Strong Connection', 'Very Strong Connection') then 1 end) as strong_collaborations,
        count(case when same_major = 1 then 1 end) as same_major_collaborations,
        count(case when collaboration_type like '%Mentoring%' then 1 end) as mentoring_opportunities,
        round(
            count(case when performance_similarity like '%Similar%' then 1 end) * 100.0 / count(*), 2
